...
async function computeBookingCost(packageId, petWeight, addOns, singleServices) {
  ...
  // Fallback to window.packagesList if available
  if (!packagesData && typeof window.packagesList !== 'undefined') {
  ...
  // If still not an array, try getPackages() if available
  if (!Array.isArray(packagesData) && typeof getPackages === 'function') {
    try {
      const pkgResult = getPackages();
      packagesData = (pkgResult && typeof pkgResult.then === 'function') ? await pkgResult : pkgResult;
    } catch (e) {
      console.warn('Failed to get packages from getPackages()', e);
    }
  }
  ...
  if (!pkg) {
    // Check if it's a single service booking
    if (packageId === 'single-service') {
      // Logic for single service cost calculation would go here if needed
...
function renderMegaCalendar(containerId, dataset = {}) {
  ...
  for (let day = 1; day <= lastDayOfMonth.getDate(); day++) {
    ...
    // Generate ISO string manually to avoid timezone conversion
  ...
  container.innerHTML = `
    ...
    const blackoutReason = capacityStatus === 'blackout' && blackout?.reason ? escapeHtml(blackout.reason) : '';
    return `
            <div class="calendar-cell day ${statusClass} ${hasEvents ? 'has-events' : ''} ${isPast ? 'past' : ''}" ${capacityStatus === 'blackout' ? 'style="background-color: var(--gray-300); color: var(--gray-500);"' : ''}>
              ...
              ${capacityStatus !== 'blackout' ? `<div class="capacity-pill" style="font-size: 0.7rem; margin-top: 0.25rem; font-weight: 600;">${slotsText}</div>` : `<div class="capacity-pill" style="font-size: 0.65rem; margin-top: 0.25rem; font-weight: 500; color: var(--gray-600);">${blackoutReason || 'Closed'}</div>`}
...
//     if (type === 'success') alertBox.style.backgroundColor = '#10b981'; // Green
//     else if (type === 'error') alertBox.style.backgroundColor = '#ef4444'; // Red
//     else if (type === 'warning') alertBox.style.backgroundColor = '#f59e0b'; // Amber
//     else alertBox.style.backgroundColor = '#3b82f6'; // Blue
...
//     }
//   },
//   success: (msg) => customAlert.show(msg, 'success'),
//   error: (msg) => customAlert.show(msg, 'error'),
//   warning: (msg) => customAlert.show(msg, 'warning'),
//   info: (msg) => customAlert.show(msg, 'info'),
